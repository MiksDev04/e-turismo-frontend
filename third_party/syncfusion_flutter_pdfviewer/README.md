...
This directory is a vendored copy of `syncfusion_flutter_pdfviewer` **33.2.13** (from the
pub cache) used via a `path` entry in `dependency_overrides` in `pubspec.yaml`.

**Patch applied (must be re-applied on any version bump):**
`lib/src/pdfviewer.dart` line ~1212 — changed the hard-coded minimum zoom:
`final double _minScale = 1;` → `final double _minScale = 0.5;`

Why: the package clamps any `PdfViewerController.zoomLevel` below 1.0 back to 1.0
(in `_handleControllerValueChange`, the `zoomLevel` property branch), so zooming out
below 100% was impossible (the report modal's minus button did nothing at 100%).
This patch lowers the floor to 0.5 (50%) to match the app's desired min zoom.
