...
import '../../business/widgets/message_view_dialog.dart';
...
// ─── Attraction Messages Page ─────────────────────────────────────────────────

class AttractionMessagesPage extends StatefulWidget {
  const AttractionMessagesPage({super.key});

  @override
  State<AttractionMessagesPage> createState() => _AttractionMessagesPageState();
}

class _AttractionMessagesPageState extends State<AttractionMessagesPage> {
  ...
  Future<void> _loadData() async {
    ...
    setState(() => _attractionId = session?.attractionId);

    if (_attractionId == null) {
      setState(() {
        _error     = 'No attraction account found for this user.';
        _isLoading = false;
      });
      return;
    }
    ...
    try {
      final type = switch (_activeFilter) {
       ...
       final result = await _api.fetchAttractionInbox(
         _attractionId!,
         page: _currentPage + 1,
         pageSize: _pageSize,
         type: type,
       );
       final unreadCount = await _api.fetchAttractionUnreadCount();
       if (mounted) {
         setState(() {
