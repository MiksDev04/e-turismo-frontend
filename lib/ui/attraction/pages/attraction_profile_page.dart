...
import '../../business/widgets/offline_state.dart';
import '../../../api/attraction_profile_api.dart';
...
// ─── Attraction Profile Page ──────────────────────────────────────────────────

class AttractionProfilePage extends StatefulWidget {
  const AttractionProfilePage({super.key});

  @override
  State<AttractionProfilePage> createState() => _AttractionProfilePageState();
}

class _AttractionProfilePageState extends State<AttractionProfilePage> {
  final _api = AttractionProfileApi();
  ...
  AttractionProfileModel? _attraction;
  ...
  List<AttractionType> _selectedTypes = [AttractionType.ecotourism];
  ...
  void dispose() {
    ...
    for (final c in [
      ...
      _attractionNameCtrl, _streetCtrl, _barangayCtrl,
    ]) { c.dispose(); }
    super.dispose();
  }
  ...
  Future<void> _loadData() async {
    ...
    try {
      final results = await Future.wait([
        _api.fetchProfile(),
        _api.fetchAttraction(),
      ]);
      if (!mounted) return;
      _profile    = results[0] as ProfileModel;
      _attraction = results[1] as AttractionProfileModel?;
      _populate();
  ...
  void _populate() {
    ...
    final a = _attraction;
    if (a == null) return;
    _attractionNameCtrl.text = a.attractionName;
    _streetCtrl.text         = a.street ?? '';
    _barangayCtrl.text       = a.barangay ?? '';
    setState(() {
      _selectedTypes = a.attractionType.isNotEmpty
          ? a.attractionType : [AttractionType.ecotourism];

      // Handle barangay selection
      if (_sanPabloBarangays.contains(a.barangay)) {
  ...
  Future<void> _saveAttractionInfo() async {
    ...
    try {
      await _api.updateAttractionInfo(
        attractionName: _attractionNameCtrl.text,
        attractionType: _selectedTypes,
  ...
  Future<void> _uploadDocuments() async {
    ...
    try {
      final url = await _api.uploadValidId(validIdFile: _selectedValidIdFile!);
      if (!mounted) return;
      final a = _attraction;
      if (a != null) {
        _attraction = AttractionProfileModel(
          id: a.id,
          userId: a.userId,
          attractionName: a.attractionName,
          attractionType: a.attractionType,
          validIdUrl: url.isNotEmpty ? url : a.validIdUrl,
          barangay: a.barangay,
          street: a.street,
          status: a.status,
          remarks: a.remarks,
                        ...
                        child: ConstrainedBox(
                          ...
                          child: Column(
                            ...
                            children: [
                              ...
                              _AttractionCard(attraction: _attraction),
                              ...
                              _AttractionInfoCard(
                                ...
                                onTypesChanged: (v) =>
                                    setState(() => _selectedTypes = v),
                                isSaving:  _isSavingAttraction,
                                onSave:    _saveAttractionInfo,
                                isNarrow:  isNarrow,
                                hasRecord: _attraction != null,
      ...
      children: [
        Column(
          ...
          children: [
            ...
            SizedBox(height: 4),
            Text(
              'Manage your account and attraction information',
...
// ─── Attraction Card (identity) ───────────────────────────────────────────────

class _AttractionCard extends StatelessWidget {
  const _AttractionCard({required this.attraction});
  final AttractionProfileModel? attraction;

  @override
  Widget build(BuildContext context) {
    final name   = attraction?.attractionName ?? '—';
    final line   = attraction?.typeLabel ?? '';
    final status = attraction?.status ?? AttractionStatus.pending;
      ...
      child: Row(
        children: [
          Container(
            ...
            child: const Center(
              child: Icon(Icons.attractions_outlined,
...
class _AttractionInfoCard extends StatelessWidget {
  ...
  final TextEditingController streetCtrl, barangayCtrl;
  final List<AttractionType> selectedTypes;
  final ValueChanged<List<AttractionType>> onTypesChanged;
  final bool isSaving;
  final VoidCallback onSave;
  final bool isNarrow;
  ...
  Widget build(BuildContext context) {
    return _SectionCard(
      child: Column(
        crossAxisAlignment: CrossAxisAlignment.start,
        children: [
          const _SectionTitle(icon: Icons.attractions_outlined, label: 'Attraction Information'),
          const SizedBox(height: 20),

          // ── Identity ─────────────────────────────────────────────────────
          const _SubLabel(label: 'Identity'),
          const SizedBox(height: 12),
          _LabeledField(label: 'Attraction Name',
              child: _InputField(controller: attractionNameCtrl)),
          const SizedBox(height: 14),
          _LabeledField(
            label: 'Attraction Type',
            child: _AttractionTypeChips(
          ...
          if (!hasRecord)
            Padding(
              ...
              child: Text(
                'No attraction record found. Please contact the admin.',
...
// ─── Attraction Documents Card ────────────────────────────────────────────────

class _AttractionDocumentsCard extends StatelessWidget {
  const _AttractionDocumentsCard({
  ...
  final AttractionProfileModel? attraction;
  final PlatformFile? selectedValidIdFile;
  final bool isSaving, hasRecord;
  final VoidCallback onPickValidIdFile, onUpload;
    ...
    return _SectionCard(
      child: Column(
        ...
        children: [
          ...
          // ── Valid ID ──────────────────────────────────────────────────────
          const _SubLabel(label: "Representative's Valid ID"),
          const SizedBox(height: 12),
          _DocumentRow(
            label: 'Valid ID',
            docTitle: "Representative's Valid ID",
            businessName: attractionName,
            fileUrl: attraction?.validIdUrl,
            fileName: _fileNameFromUrl(attraction?.validIdUrl),
...
// ─── Attraction Type Chips ────────────────────────────────────────────────────

class _AttractionTypeChips extends StatelessWidget {
  const _AttractionTypeChips({
    required this.selected,
    required this.onChanged,
  });

  final List<AttractionType> selected;
  final ValueChanged<List<AttractionType>> onChanged;
  ...
  Widget build(BuildContext context) {
    return Wrap(
      spacing: 8,
      runSpacing: 8,
      children: AttractionType.values.map((type) {
        final isOn = selected.contains(type);
        return GestureDetector(
          onTap: () {
            final next = List<AttractionType>.from(selected);
            if (isOn) {
...
class _PasswordChangeDialog extends StatefulWidget {
  ...
  final AttractionProfileApi api;
  ...
  Widget build(BuildContext context) {
    return _DialogShell(
      ...
      child: switch (_step) {
        ...
        4 => _EmailChangeConfirmationSent(
          newEmail: _newEmailCtrl.text,
          onDone: () {
...
class _EmailChangeConfirmationSent extends StatelessWidget {
  const _EmailChangeConfirmationSent({
